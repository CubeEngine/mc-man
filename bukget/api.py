from bukget.plugin import Plugin, Version
...
def get_slug(name):
    ...
    response = request.urlopen(BUKGET_PLUGIN_URL + name.lower().replace(' ', '-'), urlencode({"fields": "slug"}))
    if json.loads(response.readall().decode("utf-8")):
        return name.lower().replace(' ', '-')
    
    # Then we search for a plugin with name that matches
    response = request.urlopen(BUKGET_SEARCH_URL % ("plugin_name", "=", name), urlencode({"fields": "slug"}))
    ...
    response = request.urlopen(BUKGET_SEARCH_URL % ("plugin_name", "like", name), urlencode({"fields": "slug"}))
    json_object = json.loads(response.readall().decode("utf-8"))
    if len(json_object) > 0:
